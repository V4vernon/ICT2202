...
def Case_Add():
    ...
    if request.method == 'POST' and 'casename' in request.form and 'Assigned_To' in request.form \
        ...
        cursor.execute('INSERT INTO bitcase VALUES (NULL, %s, %s, %s, %s, %s)',
                       (casename, mydate,location, status, account_id['id']))
        mysql.connection.commit()
    ...
    if account:
        ...
        if added_items:
            for num in range(len(added_items)):
                ...
                evidence_dict['handler'] = case_data['assigned']
                evidence_dict['curr_status'] = added_items[num]["args"]["_currStatus"]
                evidence_dict['purpose'] = added_items[num]["args"]["_purpose"]
                evidence_dict['evid_added_time'] = added_items[num]["args"]["date"]
                evidence_list.append(evidence_dict.copy())
        case_dict['evidence'] = evidence_list
        final_case_dict['cases'] = case_dict
